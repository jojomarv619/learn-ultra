...
public class GameControllerForTellingTime : MonoBehaviour
{
    [SerializeField]
    private string num; //answer 
    ...
    public void GetInput()
    {
        ...
        {
            var guess = input.text +":"+ input2.text;
            StartCoroutine(CompareGuesses(guess));
            countGuess++;
            Debug.Log("Tries: " + countGuess);
        }
                        ...
                        using (UnityWebRequest www = UnityWebRequest.Post("http://learnplusultra.xyz/app_scripts/insertgamedata.php", form))
                        {
                            ...
                            else
                            {
                                ...
                                if (text != null)
                                {
                                    ...
                                    if (gameType == "dynamic_addition")
                                    {
                                        DBManager.currentDAStage = "25";
                                    }

                                    else if (gameType == "identifying_numbers")
                                    {
                                        DBManager.currentINStage = "25";
                                    }


                                    else if (gameType == "skip_counting")
                                    {
                                        DBManager.currentSCStage = "25";
                                    }

                                    else if (gameType == "telling_time")
                                    {
                                        DBManager.currentTTStage = "25";
                                    }
                        ...
                        using (UnityWebRequest www = UnityWebRequest.Post("http://learnplusultra.xyz/app_scripts/insertgamedata.php", form))
                        {
                            ...
                            else
                            {
                                ...
                                if (text != null)
                                {
                                    ...
                                    if (gameType == "dynamic_addition")
                                    {
                                        var dbDAStage = int.Parse(DBManager.currentDAStage);

                                        if (dbDAStage >= nextNumber)
                                        {
                                            DBManager.currentDAStage = dbDAStage.ToString();
                                        }

                                        else
                                        {
                                            DBManager.currentDAStage = nextNumber.ToString();
                                        }
                                    ...
                                    {
                                        var dbINStage = int.Parse(DBManager.currentINStage); //Gets current current value from DB

                                        if (dbINStage >= nextNumber) //compares if current level in DB is greater than attained current level
                                        {
                                            DBManager.currentINStage = dbINStage.ToString();
                                        }

                                        else
                                        {
                                            DBManager.currentINStage = nextNumber.ToString();
                                        }
                                        // DBManager.currentINStage = nextNumber.ToString();
                                    }
                                    // DBManager.currentActiveStage = nextSceneNumber;
                                    ...
                                    {
                                        var dbSCStage = int.Parse(DBManager.currentSCStage);

                                        if (dbSCStage >= nextNumber)
                                        {
                                            DBManager.currentSCStage = dbSCStage.ToString();
                                        }

                                        else
                                        {
                                            DBManager.currentSCStage = nextNumber.ToString();
                                        }
                                    ...
                                    {
                                        var dbTTStage = int.Parse(DBManager.currentTTStage);

                                        if (dbTTStage >= nextNumber)
                                        {
                                            DBManager.currentTTStage = dbTTStage.ToString();
                                        }

                                        else
                                        {
                                            DBManager.currentTTStage = nextNumber.ToString();
                                        }
                                    }
                                }
                        ...
                        using (UnityWebRequest www = UnityWebRequest.Post("http://learnplusultra.xyz/app_scripts/insertgamedata.php", form))
                        {
                            ...
                            else
                            {
                                ...
                                if (text != null)
                                {
                                    ...
                                    {
                                        var dbINStage = int.Parse(DBManager.currentINStage);

                                        if (dbINStage >= nextNumber)
                                        {
                                            DBManager.currentINStage = dbINStage.ToString();
                                        }
